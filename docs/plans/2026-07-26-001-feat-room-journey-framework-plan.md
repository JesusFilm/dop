...
- **Objective:** Add the reusable journey framework that carries each room from assignment reveal through a coordinator-led sequence of synchronized module instances.
...
Every room follows the same journey while its coordinator controls synchronized, forward-only progression for that room.

### Problem Frame

The current Day of Prayer experience ends after participants learn their room, group, and coordinator.
...
- **Let the room coordinator control forward-only progression.** (session-settled: user-directed — chosen over automatic transitions, individual progression, or backward navigation: each room should remain together while moving at its own pace.) Governs R8–R13.
- **Use synchronized timers as guidance only.** (session-settled: user-directed — chosen over enforced durations or automatic advancement: the coordinator may move forward at any time.) Governs R10–R12.
- **Extend the existing reveal into the journey launch.** (session-settled: user-approved — chosen over a second organizer start control: reveal should move rooms into an untimed gathering state before coordinators begin.) Governs R5–R7.
...
- A2. **Room coordinator:** Starts the first module and advances the room through the journey.
...
- R7. Only the room coordinator can start the first module once the physical room is ready.

**Room progression and timing**

- R8. Every participant in a room sees the same current module instance.
- R9. Only the current room coordinator can advance the room to the next module instance.
- R10. Entering a timed module starts one synchronized countdown from that module instance’s recommended duration.
- R11. The countdown never advances the room automatically, and the coordinator may advance before or after it reaches zero.
...
- R18. Coordinator takeover preserves the room’s current module, timer, and module-specific state.
...
stateDiagram-v2
  ...
  Gathering --> Module: coordinator starts first module
  Module --> Module: coordinator advances
  Module --> Complete: coordinator advances from final module
...
- F1. Launch and start a room journey
  ...
  - **Actors:** A2, A3
  - **Steps:** Reveal moves every assigned room to the gathering state; each coordinator starts the first module after their room is physically ready.
  - **Outcome:** Every room begins the configured journey without a second organizer launch.
  - **Covers:** R5–R8.

- F2. Advance a room through modules
  - **Trigger:** A coordinator starts or advances their room.
  - **Actors:** A1, A2
  - **Steps:** All room screens enter the same module and timer; the coordinator advances when appropriate; other rooms remain unaffected.
...
- F4. Complete or reset a journey
  - **Trigger:** A coordinator advances from the final module, or the organizer resets the gathering.
  - **Actors:** A1, A2, A3
...
- AE1. Reveal does not start a timed prayer activity
  ...
  - **Then:** Every participant sees the untimed gathering state for their room, and no module timer starts until that room’s coordinator begins.

- AE2. Rooms progress at different speeds
  - **Covers:** R8–R13.
  - **Given:** Two rooms are on the same module instance.
  - **When:** One coordinator advances before the recommended timer ends.
...
- AE3. A module exceeds its recommended duration
  ...
  - **Then:** The room remains synchronized on that module until the coordinator advances.
...
- The current room assignment, coordinator takeover, participant continuity, polling, reveal, and reset behavior remain authoritative.
...
- KTD3. **Extend the serialized gathering lifecycle for journey mutations.** Reveal initialization, first-module start, forward advancement, coordinator takeover, late arrival, and reset use the existing PostgreSQL transaction and gathering-row lock so each room has one authoritative state. Supports R5–R19 and R23.
...
flowchart TB
  ...
  Coordinator["Room coordinator"] --> Advance["Start or advance mutation"]
  Advance --> Lifecycle
```
...
stateDiagram-v2
  ...
  Gathering --> Active: coordinator starts
  Active --> Active: coordinator advances
...
3. Extend participant and organizer projections plus coordinator mutations.
...
- **Concurrency:** Coordinator start and advance join reveal, takeover, late arrival, and reset under the existing serialized mutation boundary.
...
- **Concurrent coordinator actions:** PostgreSQL serialization and a current-instance check prevent duplicate requests from skipping a module.
...
**Goal:** Add reveal initialization, coordinator start, forward advancement, independent completion, continuity, and reset to the authoritative gathering service.
...
2. Add one coordinator-authorized forward mutation that starts the first instance, advances to the next current live instance, or marks the room complete; require the caller's expected current state so a replay cannot advance twice.
...
5. Make retries and concurrent coordinator submissions idempotent at the current-instance boundary.
...
3. Covers AE3. Passing the recommended end time does not change the current module until the coordinator advances.
...
9. A non-coordinator cannot start or advance a room.
...
11. Coordinator takeover during an active module preserves the current instance and start time.
...
### U3. Expose journey state and coordinator advancement

**Goal:** Carry journey readiness and room runtime through audience-specific snapshots and a same-origin coordinator mutation.
...
1. Extend the participant snapshot with room handoff, untimed gathering, active module, and completed states while retaining room identity, roster, and coordinator data.
...
4. Add a same-origin cookie-authenticated mutation carrying the caller's expected journey state; delegate coordinator authorization, replay protection, and transition rules to the domain service.
...
**Patterns to follow:** Reuse participant-cookie hashing in `src/app/api/participant/coordinator/route.ts`, no-store response headers in `src/app/api/participant/route.ts`, and `errorResponse` from `src/lib/gathering/http.ts`.
...
3. A coordinator advance request returns the updated participant snapshot.
4. A non-member, non-coordinator, stale, malformed, oversized, or cross-origin request cannot advance the room.
...
**Goal:** Present the untimed gathering state, registered module shell, advisory countdown, coordinator controls, late-arrival orientation, and completed screen.
...
4. Show start or continue actions only to the current coordinator and refresh the authoritative snapshot after mutation.
...
**Patterns to follow:** Reuse `ParticipantHeader`, `ActionButton`, the coordinator takeover affordance in `src/components/participant/room-assignment.tsx`, and `useLiveSnapshot` for convergence and reconnect feedback.

**Test scenarios:**

1. A future start time, active duration, zero boundary, and overtime state produce stable countdown labels.
2. The gathering state gives only the coordinator a start action while every member sees the same untimed instructions.
3. The active shell gives only the coordinator a continue action before, at, and after zero.
...
**Verification:** Unit tests cover timer boundaries, browser acceptance covers coordinator/member controls and synchronized state, and accessibility checks cover status announcements and action labels.
...
| Browser acceptance      | U4, U5     | Organizer reveal, room gathering, coordinator progression, member synchronization, late arrival, completion, compatibility, and reset are visibly correct on desktop and mobile. |
...
- U3 is done when participant and organizer contracts expose only their required journey state and the coordinator mutation enforces membership and same-origin rules.
- U4 is done when the participant shell renders gathering, active, and completed states; coordinator controls and countdowns synchronize across devices.
...
- Existing room assignment, prayer-request privacy, coordinator takeover, late arrival, and reset behavior remain green.
