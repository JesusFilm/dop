...
### App-clock authority
The principle that the running application's own clock — not any scheduler or trigger — is the sole authority for whether the Reveal time has passed. Schedulers may drift and only *nudge* computation; the sharp boundary is always the app's own comparison of now against the Reveal time.
